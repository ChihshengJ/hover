...
/** One node of the laid-out tree, in overlay pixel coordinates. */
interface TrailLayoutNode {
  node: TrailNode;
  x: number;
  y: number;
  children: TrailLayoutNode[];
  /** Height of this node plus everything under it. */
...
export class TrailOverlay {
  ...
  pillsContainer: HTMLDivElement | null;
  isPanning: boolean;
  wasDragged: boolean;
  panStartX: number;
  panOffsetX: number;
  currentTrails: Trail[];
  otherTrails: Trail[];
  allDisplayTrails: Trail[];
  bodyRow: HTMLDivElement;
  sidebar: HTMLDivElement;
  deleteBtn: HTMLButtonElement;
  svgGroup: SVGGElement | null;

  /**
   * @param currentTitle — detected title of the current paper
   */
  constructor(
    trailStore: TrailStore,
    currentTitle: string | null,
  ...
  #resolveTrails() {
    ...
    const currentTrailIds = new Set(
      locations.map((l: TrailLocation) => l.trailId),
    );
  ...
  #createOverlay() {
    ...
    this.treeContainer.addEventListener("pointerdown", (e: PointerEvent) =>
      this.#onPanStart(e),
    );
    ...
    this.treeContainer.addEventListener(
      ...
      (e: WheelEvent) => {
        ...
        if (
          e.deltaY > 0 &&
          this.currentTrailIndex < this.allDisplayTrails.length - 1
        ) {
          this.currentTrailIndex++;
          this.panOffsetX = 0;
          this.#renderCurrentTrail();
  ...
  #renderTree(trail: Trail) {
    ...
    let maxX = 0;
    let maxY = 0;
    this.#walkLayout(layout, (n: TrailLayoutNode) => {
   ...
   *
   * @param depth horizontal depth level
   * @param yOffset vertical offset for this subtree
   */
  #layoutTree(
    node: TrailNode,
    depth: number,
    ...
    const childLayouts: TrailLayoutNode[] = [];
  ...
  #walkLayout(layout: TrailLayoutNode, fn: (n: TrailLayoutNode) => void) {
    fn(layout);
  ...
  #renderConnections(group: SVGGElement, layout: TrailLayoutNode) {
    for (const child of layout.children) {
  ...
  #renderNodes(group: SVGGElement, layout: TrailLayoutNode, trail: Trail) {
    const { node } = layout;
    const isCurrent =
   ...
   */
  async #navigateToNode(node: TrailNode) {
    if (!node.url) return;
