...
import type { SearchMatch } from "./search_controller.js";
...
export class SearchHighlightLayer {
  ...
  #highlightGroups: Map<string, SVGGElement> = new Map();
  ...
  #matchesByPage: Map<number, SearchMatch[]> = new Map();

  #renderedPages: Set<number> = new Set();

  #pageLayoutCache: Map<number, { top: number; left: number }> = new Map();

  #layoutCacheValid: boolean = false;
   ...
   */
  #getPageLayout(pageNumber: number): { top: number; left: number } | null {
   ...
   * @param pageNumbers Page numbers to read layouts for
   */
  #batchReadLayouts(pageNumbers: Set<number>) {
   ...
   * @param matchId Match ID to focus, or null to clear focus
   */
  setFocus(matchId: string | null) {
   ...
   */
  getMatchRect(matchId: string): DOMRect | null {
    const match = this.#matches.find((m) => m.id === matchId);
  ...
  getMatchPosition(
    matchId: string,
  ): { pageNumber: number; offsetTop: number } | null {
    const match = this.#matches.find((m) => m.id === matchId);
    if (!match) return null;
