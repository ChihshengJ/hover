...
import type { FileMenu } from "../controls/file_menu.js";
import type { FloatingToolbar } from "../controls/floating_toolbar/index.js";

/**
 * Onboarding Walkthrough for Hover PDF Viewer
 *
 * @typedef {SplitWindowManager} SplitWindowManager
 * @typedef {FloatingToolbar} FloatingToolbar
 * @typedef {FileMenu} FileMenu
 */
...
export interface SpotlightConfig {
  ...
  y: number;
  /** Width of spotlight area */
  width: number;
  /** Height of spotlight area */
  height: number;
  /** Border radius for rounded corners; defaults to 8. */
  borderRadius?: number;
}
...
export interface TooltipConfig {
  ...
  subtext?: string;
  /** Position relative to spotlight; defaults to "bottom". */
  position?: "top" | "bottom" | "left" | "right" | "center";
  /** X offset from the default position; defaults to 0. */
  offsetX?: number;
  /** =0] - Y offset from default position */
  offsetY?: number;
  /** =false] - Show a "Next" button */
  showNextButton?: boolean;
  /** =true] - Show skip button */
  showSkipButton?: boolean;
  /** ='Next'] - Custom text for next button */
...
export interface StepConfig {
  ...
  id: string;
  /** Step type: 'message' | 'highlight' | 'action' | 'interactive' */
  type: string;
  /** Spotlight configuration (null for full overlay or no overlay) */
  spotlight?: SpotlightConfig;
  /** Tooltip configuration */
  tooltip: TooltipConfig;
  /** Event/state to wait for before auto-advancing */
  waitFor?: string;
  ...
  /** =false] - If true, hide the overlay for this step */
  noOverlay?: boolean;
}
...
export class OnboardingWalkthrough {
  ...
  fullOverlay: HTMLDivElement | null;
  tooltip: HTMLDivElement | null;
  skipButton: HTMLButtonElement | null;
  indicatorContainer: HTMLDivElement | null;
  activeListeners: Map<any, any>;
  autoAdvanceTimer: ReturnType<typeof setTimeout> | null;

  /**
   * @param wm Window manager instance
   * @param fileMenu File menu instance
   */
  constructor(wm: SplitWindowManager, fileMenu: FileMenu) {
    this.wm = wm;
  ...
  #toolLabels: HTMLElement[] = [];

  #circleIndicator: HTMLElement | null = null;

  #mutationObserver: MutationObserver | null = null;

  #searchRangeFocusCleanup: Function | null = null;
   ...
   */
  static saveIntendedUrl(url: string) {
   ...
   */
  static getAndClearIntendedUrl(): string | null {
   ...
   */
  static getDefaultPaperUrl(): string {
   ...
   */
  setStepSpotlight(stepId: string, config: SpotlightConfig) {
   ...
   */
  #showSpotlightOverlay(config: SpotlightConfig) {
    this.fullOverlay.classList.remove("visible");
   ...
   */
  #updateSpotlight(config: SpotlightConfig) {
    if (!config) return;
  ...
  #updateTooltip(
    ...
    const subtextEl = this.tooltip.querySelector(
      ".onboarding-tooltip-subtext",
    ) as HTMLElement;
   ...
   * @param position 'center' takes the default (bottom) arm
   */
  #positionTooltip(
    position: "top" | "bottom" | "left" | "right" | "center",
    spotlightConfig: SpotlightConfig | null,
   ...
   */
  #setupWaitForListener(eventName: string) {
    ...
    switch (eventName) {
      ...
      case "abstractTabClicked": {
        ...
        const handler = (e: Event) => {
          if (
            (e.target as Element).closest('[data-tab="abstract"]') ||
            (e.target as Element).textContent?.includes("Abstract")
          ) {
      ...
      case "outlinePinClicked": {
        ...
        const handler = (e: Event) => {
          if (
            (e.target as Element).closest(".nav-tree-toggle") ||
            (e.target as Element).closest(".tree-toggle") ||
            (e.target as Element).closest(".nav-node-toggle") ||
   ...
   */
  #removeWaitForListener(eventName: string) {
   ...
   */
  #showCircleIndicator(element: HTMLElement): HTMLElement {
    const rect = element.getBoundingClientRect();
   ...
   */
  #showToolButtonLabels(): HTMLElement[] {
  ...
  #onCitationReferenceEnter() {
    ...
    setTimeout(() => {
      const popup = document.querySelector(".citation-popup") as HTMLElement;
      if (popup) {
