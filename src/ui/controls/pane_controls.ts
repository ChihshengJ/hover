...
export interface PaneControlsOptions {
  /** control strip is appended here */
  paneEl: HTMLElement;
  /** the scrolling element, once it exists */
  ...
  fit: (mode: number, target: number) => void;
  toggleHandMode: () => void;
  scrollToRelative: (delta: number) => void;
}

/** The `data-action` of every button in the per-pane control strip. */
...
export class PaneControls {
  ...
  _scrollBound: boolean;
  _scrollRAF: number | null;
  scrollCallbacks: Set<any>;

  constructor(options: PaneControlsOptions) {
  ...
  attach() {
    ...
    this.element.addEventListener("click", (e) => {
      ...
      if (btn?.dataset.action) {
        this.#handleAction(btn.dataset.action as PaneAction);
      }
    });
  ...
  #handleAction(action: PaneAction) {
    switch (action) {
      case "prev":
  ...
  #updateModeToggle(isHandMode: boolean) {
