 ...
 */
/** Which end of the document an indicator jumps to: 1 is top, 0 is bottom. */
export type JumpZone = 0 | 1;

export interface JumpIndicatorOptions {
  wrapper: HTMLElement;
  isDragging: () => boolean;
  /** Run the jump the released zone stands for. */
  onExecute: (zone: JumpZone) => void;
}

export class JumpIndicators {
  wrapper: JumpIndicatorOptions["wrapper"];
  isDragging: JumpIndicatorOptions["isDragging"];
  onExecute: (zone: JumpZone) => void;
  container: HTMLDivElement | null;
  topIndicator: HTMLElement | null;
  bottomIndicator: HTMLElement | null;
  activeZone: JumpZone | null;
   ...
   */
  constructor({ wrapper, isDragging, onExecute }: JumpIndicatorOptions) {
