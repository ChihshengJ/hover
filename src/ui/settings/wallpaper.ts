...
import type { CustomWallpaper, WallpaperMeta } from "./config.js";

/** A wallpaper that ships with the app, fetched on first use. */
...
export class WallpaperManager {
  ...
  _meta: WallpaperMeta;
  _activeObjectUrl: string | null;
  _pendingPresetThumbs: Set<any>;

  static DB_NAME: string = "HoverWallpaperDB";
  static DB_VERSION: number = 1;
  static STORE_NAME: string = "wallpapers";
   ...
   */
  static PRESETS: WallpaperPreset[] = [
    {
      id: "preset_tiger",
   ...
   */
  async setPersistInNight(enabled: boolean) {
   ...
   * @param id wallpaper ID or null to clear
   */
  async selectWallpaper(id: string | null) {
    ...
    if (!id) {
      ...
      try {
        ...
        if (blob) {
          this._applyWallpaperBlob(blob);
          const entry = this._meta.custom.find(
            (c: CustomWallpaper) => c.id === id,
          );
   ...
   */
  async addFromFile(file: File): Promise<boolean> {
   ...
   */
  async deleteCustom(ids: Set<string>) {
    for (const id of ids) {
      await this._deleteImage(id);
      this._meta.custom = this._meta.custom.filter(
        (c: CustomWallpaper) => c.id !== id,
      );
    }
   ...
   * @param preset { id, name, url }
   * @param card the card DOM element to update
   */
  async ensurePresetThumbnail(preset: Record<string, any>, card: HTMLElement) {
  ...
  _ensureDB(): Promise<IDBDatabase> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        reject(new Error(`IndexedDB not available: ${err}`));
        return;
      }
   ...
   */
  _putImage(id: string, blob: Blob): Promise<void> {
    return new Promise((resolve, reject) => {
   ...
   */
  _getImage(id: string): Promise<Blob | null> {
    return new Promise((resolve, reject) => {
   ...
   */
  _deleteImage(id: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const tx = this._db.transaction(WallpaperManager.STORE_NAME, "readwrite");
      const store = tx.objectStore(WallpaperManager.STORE_NAME);
      store.delete(id);
   ...
   * @param src data URL, blob URL, or remote URL
   */
   ...
   * @param src original data URL, object URL, or remote URL
   */
  async _processImage(src: string): Promise<{ full: Blob; thumb: string }> {
   ...
   */
  async _generatePresetThumbnail(url: string): Promise<string> {
    try {
