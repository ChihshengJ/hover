...
export interface AutoHideOptions {
  wrapper: HTMLElement;
  isTreeOpen: () => boolean;
  /** Collapse the toolbar before it slides out of view. */
  onSlideOutCollapse: () => void;
}

export class AutoHideController {
  wrapper: AutoHideOptions["wrapper"];
  isTreeOpen: AutoHideOptions["isTreeOpen"];
  onSlideOutCollapse: AutoHideOptions["onSlideOutCollapse"];
  isHidden: boolean;
   ...
   */
  constructor({ wrapper, isTreeOpen, onSlideOutCollapse }: AutoHideOptions) {
  ...
  reposition(wrapperCenterY: number) {
