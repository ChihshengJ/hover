...
import type { SplitDirection } from "../../viewer/window_manager.js";

/** A stop in the floating ball's gradient. */
...
export interface BallStyle {
  gradient: { direction: number; stops: GradientStop[] };
  pageColor: string;
  pageWeight: number;
...
/** One wallpaper the user added; the bytes live in IndexedDB under `id`. */
export interface CustomWallpaper {
  id: string;
  name: string;
  /** Data URL of the small preview. */
  thumbnail: string;
}
...
export interface WallpaperMeta {
  ...
  /** presetId → thumbnail data URL. */
  presetThumbs: Record<string, string>;
  persistInNight: boolean;
}

/**
 * - `day` — always launch in day mode (the file-menu toggle still works at
 *   runtime, but is not persisted)
 * - `night` — always launch in night mode, same caveat
 * - `persist` — remember the last toggled mode across launches
 */
export type NightModeStartup = "day" | "night" | "persist";
...
/**
 * Every preference, and the type of its value.
 *
 * This is the one declaration `Config.get`/`set`/`subscribe` are generic over,
 * so a typo'd key is a compile error and a read comes back with a real type
 * rather than `any`.
 */
export interface ConfigValues {
  progress_bar_enabled: boolean;
  toolbar_auto_collapse: boolean;
  liquid_glass_enabled: boolean;
  default_tool: string;
  wallpaper_meta: WallpaperMeta;
  ball_style: BallStyle;
  night_mode_startup: NightModeStartup;
  night_mode_last: boolean;
  split_persist: boolean;
  split_state: SplitState | null;
}

export type ConfigKey = keyof ConfigValues;

/** One schema row: where the value is stored, and what it is when unset. */
type SchemaEntry<K extends ConfigKey> = {
  storageKey: string;
  default: ConfigValues[K];
};

const DEFAULT_WALLPAPER_META: WallpaperMeta = {
  activeId: null,
  custom: [],
  presetThumbs: {},
...
const DEFAULT_BALL_STYLE: BallStyle = {
  gradient: {
    direction: 120,
    stops: [
 ...
 */
const SCHEMA: { [K in ConfigKey]: SchemaEntry<K> } = {
  // Display
  ...
  // See NightModeStartup for what the three values mean.
  night_mode_startup: {
...
export class Config {
  static _cache: Partial<ConfigValues> | null = null;
  static _listeners = new Map<ConfigKey, Set<(value: any) => void>>();
  ...
  static async load() {
    ...
    this._cache = cache as Partial<ConfigValues>;
  }

  static get<K extends ConfigKey>(key: K): ConfigValues[K] {
    const def = SCHEMA[key];
    if (!def) {
      console.warn(`[Config] get('${key}') — unknown key`);
      return undefined as ConfigValues[K];
    }
    if (!this._cache) {
      console.warn(
    ...
    return this._cache[key] as ConfigValues[K];
  }

  static async set<K extends ConfigKey>(key: K, value: ConfigValues[K]) {
    const def = SCHEMA[key];
    if (!def) {
  ...
  static subscribe<K extends ConfigKey>(
    key: K,
    cb: (value: ConfigValues[K]) => void,
  ): () => void {
  ...
  static _emit(key: ConfigKey, value: unknown) {
   ...
   */
  static _readAll(storageKeys: string[]): Promise<Record<string, any>> {
   ...
   * JSON-encoded blobs.
   */
  static _readAllLocalStorage(storageKeys: string[]): Record<string, any> {
  ...
  static _writeOne(storageKey: string, value: unknown): Promise<void> {
    try {
