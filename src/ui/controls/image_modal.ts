...
let instance: ImageModal | null = null;
...
/** Anything that can produce the pixels the modal displays. */
...
class ImageModal {
  ...
  /**
   * Narrower than `ImageObjectInfo`, which the JSDoc used to name: the modal
   * reads nothing but the pixels, and the live caller (region select) hands it
   * a bare closure over a freshly rendered `ImageData`.
   */
  show(imageInfo: PixelSource) {
  ...
  _onWheel(e: WheelEvent) {
    ...
    const newScale = Math.min(
      MAX_SCALE,
      Math.max(MIN_SCALE, this._scale * (1 + delta)),
    );
  ...
  #applyTransform() {
    this.img.style.transform = `translate(${this._translateX}px, ${this._translateY}px) scale(${this._scale})`;
  }
