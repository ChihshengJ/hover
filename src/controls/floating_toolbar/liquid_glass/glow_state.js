...
export class GlowState {
  /**
   * @param {Object} opts
   * @param {number} opts.cx Rest x, canvas-local px (ball center).
   * @param {number} opts.cy Rest y, canvas-local px (ball center).
   */
  constructor({ cx, cy }) {
    this.x = cx;
    this.y = cy;
   ...
   * @param {number} bx Blob center x, canvas-local px.
   * @param {number} by Blob center y, canvas-local px.
   * @param {number} br Blob radius, px.
   */
  setLeash(bx, by, br) {
    this.leashX = bx;
    this.leashY = by;
  ...
  /** @param {boolean} on Pointer is pressed / dragging. */
  setPressed(on) {
    this.pressed = on;
  }
   ...
   * @param {number} dt Seconds. Clamped to match GooState's tunneling guard.
   */
  step(dt) {
    dt = Math.min(dt, 1 / 30);
