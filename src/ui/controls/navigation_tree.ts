export interface TreeNode {
  id: string;
  title: string;
  type: "section" | "figure" | "table" | "annotation";
  /** 0-based page index */
  pageIndex: number;
  /** X position in PDF coordinates */
  left: number;
  /** Y position in PDF coordinates (origin bottom-left, higher = higher on page) */
  top: number;
  /** Column index: -1 for full-width, 0 for col1, 1 for col2, etc. */
  columnIndex: number;
  children: TreeNode[];
  expanded: boolean;
  /** 'highlight' or 'underline' */
  annotationType?: string;
  /** annotation coloroutline_builder */
  color?: string;
  /** reference to actual annotation */
  annotationId?: string;
  /** e.g., "pp. 3-5" for cross-page */
  pageRange?: string;
  /** dots to show on section */
  annotationDots?: { color: string }[];
  /** count beyond 3 dots */
  extraAnnotationCount?: number;
  /** annotation's top as a fraction of page height */
  originalTopRatio?: number;
}
...
/**
 * A PDF destination resolved to a page and a position on it, or `null` when it
 * could not be resolved — cached either way, so a bad dest is only chased once.
 */
/** One laid-out glyph run, for matching a link annotation to the text under it. */
...
export interface NavigationTreeOptions {
  ...
  getPane: () => ViewerPane;
  /** viewport y of the toolbar ball's centre */
  getBallCenterY: () => number;
}
...
export class NavigationTree {
  ...
  tree: TreeNode[];
  flatSections: TreeNode[];
  pageDimensions: Array<{ width: number; height: number }>;
  isVisible: boolean;
  treeBuilt: boolean;
  backdrop: HTMLDivElement | null;
  container: HTMLDivElement | null;
  treeWrapper: HTMLDivElement | null;
  branchSvg: SVGSVGElement | null;
  TREE_WIDTH: number;
  NODE_HEIGHT: number;
  INDENT: number;
  BRANCH_COLOR: string;
  ...
  constructor(options: NavigationTreeOptions) {
  ...
  async #cachePageDimensions() {
    ...
    const pdfDoc: any = this.doc.pdfDoc;
    if (!pdfDoc) return;
   ...
   * @returns Page height in PDF units (default 792 for letter size)
   ...
   * @returns Page width in PDF units (default 612 for letter size)
   * @param pageIndex 0-based page index
   */
   ...
   * @param outlineItems Items from doc.outline
   */
  #convertOutlineToTreeNodes(outlineItems: OutlineItem[]): TreeNode[] {
    return outlineItems.map((item): TreeNode => ({
      id: item.id,
      title: item.title,
      type: "section",
  ...
  async #resolveDestination(
    dest: unknown,
    cache: Map<string, ResolvedDestination>,
    ...
    if (typeof dest === "string") {
      explicitDest = namedDests?.get(dest);
      if (!explicitDest) {
        try {
   ...
   */
  #flattenSections(items: TreeNode[], result: TreeNode[] = []): TreeNode[] {
    for (const item of items) {
  ...
  async #extractFigureTableAnnotations(
    destCache: Map<string, ResolvedDestination>,
  ) {
    const items = [];
    // FIXME(pdfium-migration): PDF.js API on a PDFium document. `numPages`,
    // `getPage`, `getViewport`, `getAnnotations`, `getTextContent`,
  ...
  #buildTextSpatialIndex(textContent: { items: any[] }): SpatialTextItem[] {
  ...
  #findAnnotationText(annot: any, textIndex: SpatialTextItem[]) {
  ...
  #insertFiguresIntoTree(figureItems: TreeNode[]) {
   ...
   * @returns The section that precedes this position in reading order
   * @param pageIndex 0-based page index
   * @param pdfY Y position in PDF coordinates (higher = higher on page)
   * @param columnIndex Column index: -1 for full-width, 0+ for columns
   */
   ...
   * @returns True if section comes before the target in reading order
   * @param section The section to check
   * @param targetPageIndex Target page index
   * @param targetPdfY Target Y in PDF coordinates
   * @param targetColumn Target column index (-1 for full-width)
   */
  #isSectionBefore(
    section: TreeNode,
   ...
   * @returns Column index: -1 for full-width/single-column, 0 for left, 1+ for further columns
   * @param pageIndex 0-based page index
   * @param leftX X position in PDF coordinates (absolute, not ratio)
   */
  #getColumnIndex(pageIndex: number, leftX: number): number {
  ...
  #clearAnnotationsFromTree(nodes: TreeNode[]) {
   ...
   * @param counter Annotation counter for display
   */
   ...
   * @param annotationNode The annotation node to insert
   */
  #insertAnnotationIntoTree(annotationNode: TreeNode) {
    const section = this.#findContainingSection(
  ...
  #calculateAnnotationDots(nodes: TreeNode[]) {
    for (const node of nodes) {
      if (node.type === "section") {
        const annotations = this.#collectAnnotationsInSubtree(node);
        const colors = annotations.map((a: TreeNode) => a.color);
  ...
  #collectAnnotationsInSubtree(node: TreeNode): TreeNode[] {
    const annotations = [];
  ...
  #collapseSiblings(wrapper: HTMLElement, currentNode: TreeNode) {
  ...
  #expandNode(node: TreeNode, wrapper: HTMLElement, path: number[]) {
    ...
    const childrenContainer = wrapper.querySelector<HTMLElement>(
  ...
  #collapseNode(node: TreeNode, wrapper: HTMLElement) {
    if (!node.expanded) return;
  ...
  #collapseAllChildren(node: TreeNode) {
    for (const child of node.children) {
      if (child.expanded) {
  ...
  #togglePin(node: TreeNode, wrapper: HTMLElement, path: number[]) {
  ...
  #isInPinnedPath(nodeId: string): boolean {
    return this.pinnedPath.includes(nodeId);
  }

  /**
   * The chain of node **ids** from the root down to `targetId`.
   *
   * Not the same "path" as the one `#createNodeElement` and `#expandNode`
   * take: those are child indices into the tree. This one is what
   * `#isInPinnedPath()` tests membership against.
   */
  #getPathToNode(
    targetId: string,
    ...
    for (const node of nodes) {
      ...
      if (node.children.length > 0) {
        const found: string[] | null = this.#getPathToNode(
          targetId,
          node.children,
          newPath,
        );
  ...
  #getAnnotationColor(colorName: string): string {
    const colors = {
      yellow: "#f59e0b",
      red: "#ef4444",
      blue: "#3b82f6",
    ...
    return colors[colorName as keyof typeof colors] || colors.yellow;
  }
  ...
  #drawAllBranches() {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        if (rootList) {
          const rootItems = rootList.querySelectorAll<HTMLElement>(
            ":scope > .nav-tree-item",
          );
        ...
        const expandedItems = this.treeWrapper.querySelectorAll<HTMLElement>(
          ".nav-tree-item.expanded",
        );
        expandedItems.forEach((parentItem) => {
          const childrenContainer = parentItem.querySelector<HTMLElement>(
            ":scope > .nav-tree-children",
          );
          if (!childrenContainer) return;

          const childItems = childrenContainer.querySelectorAll<HTMLElement>(
            ":scope > .nav-tree-item",
          );
  ...
  #findNodeById(id: string, nodes: TreeNode[]): TreeNode | null {
    for (const node of nodes) {
      if (node.id === id) return node;
      if (node.children.length > 0) {
        const found: TreeNode | null = this.#findNodeById(id, node.children);
        if (found) return found;
      }
    }
    return null;
  ...
  async show(ballRightX: number, onClose: (() => void) | null = null) {
   ...
   */
  onDocumentChange(event: DocEventName) {
  ...
  #centerOnBall() {
    ...
    (wrapper as HTMLElement).style.top = `${clampedTop}px`;
