 ...
 */
const PROFILES = {
  circle: (x: number) => Math.sqrt(1 - (1 - x) * (1 - x)),
  squircle: (x: number) => Math.pow(1 - Math.pow(1 - x, 4), 0.25),
  concave: (x: number) => 1 - Math.sqrt(Math.max(0, 1 - x * x)),
};

/** Which cross-section the bezel dome follows. */
export type BezelProfile = keyof typeof PROFILES;
...
export interface BezelLUTOptions {
  profile?: BezelProfile;
  /** Band width inside the edge, px. */
  bezelWidth: number;
  /** Surface rise across the band, px. */
  bezelHeight: number;
  /** Flat glass depth under the dome, px. */
  thickness: number;
  /** n₂ of the glass (n₁ = 1, air). */
 ...
 *
 * @returns Displacement is positive inward (convex profiles pull the backdrop
 *   toward the shape's center).
 */
export function buildBezelLUT({
  ...
  refractiveIndex = 1.5,
  samples = 128,
}: BezelLUTOptions): { lut: Float64Array; maxDisp: number } {
 ...
 * outside the browser.
 *
 * @returns RGBA, row-major.
 */
export interface DisplacementFieldOptions {
  /** Square map size, CSS px. */
  cssSize: number;
  /** Raster pixels per CSS px. */
  res?: number;
  /** Signed distance, CSS px, y-down, negative inside. */
  sdf: (x: number, y: number) => number;
  /** Bezel displacement lookup table, from `buildBezelLUT()`. */
  lut: Float64Array;
  /** Peak displacement, CSS px. */
  maxDisp: number;
  /** Depth of the refracting band, CSS px. */
  bezelWidth: number;
  /**
   * Reused output buffer (every pixel is overwritten). Allocated fresh when
   * omitted.
   */
...
export function computeDisplacementField({
  ...
  bezelWidth,
  out,
}: DisplacementFieldOptions): Uint8ClampedArray {
 ...
 * @returns null when the LUT is
 *   degenerate (zero displacement everywhere).
 * @param opts `res` defaults to 2 for a crisp map on retina displays.
 */
export function renderDisplacementMap(
  opts: Omit<DisplacementFieldOptions, "out">,
