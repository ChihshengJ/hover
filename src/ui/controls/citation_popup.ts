...
import type {
  Citation,
  CitationTarget,
} from "../../analysis/citation_builder.js";

/** Which half of the popup is showing. */
export type CitationTab = "reference" | "abstract";

/** One Google Scholar hit, as `#parseScholarResult()` reads it out of the page. */
...
export class CitationPopup {
  ...
  scholarData: ScholarResult | null;
  scholarError: string | null;
  isLoadingScholar: boolean;
  citation: Citation | null;
  allTargets: CitationTarget[];
  currentTargetIndex: number;
  findCiteTextCallback:
    ((target: CitationTarget | null) => Promise<string | null>) | null;
   ...
   * @param anchor The citation element being hovered
   * @param citation Full citation object with allTargets
   * @param findCiteTextCallback Callback to find reference text
   * @param initialTargetIndex If set, start at this target (for per-number overlays)
   */
  async show(
    anchor: HTMLElement,
    citation: Citation,
    ...
    initialTargetIndex: number | null = null,
  ...
  async showWithText(anchor: HTMLElement, text: string) {
  ...
  #switchTab(tabName: CitationTab) {
    if (this.activeTab === tabName) return;

    this.activeTab = tabName;

    // Update tab button states
    const tabs = this.popup.querySelectorAll<HTMLElement>(".citation-tab");
    tabs.forEach((tab: HTMLElement) => {
      tab.classList.toggle("active", tab.dataset.tab === tabName);
    });

    const content = this.popup.querySelector<HTMLElement>(
  ...
  #renderReferenceContent(container: HTMLElement, text: string) {
    container.innerHTML = "";
  ...
  #updateTextDisplay(
    ...
    callback: (el: HTMLElement, text: string) => void,
  ) {
    if (this.isExpanded) {
  ...
  async #renderAbstractContent(container: HTMLElement) {
    container.innerHTML = "";
    ...
    if (!this.isLoadingScholar) {
      ...
      try {
        ...
        this.scholarError = message;
        this.#renderScholarError(container, message);
      } finally {
        this.isLoadingScholar = false;
        this.positionPopup();
      }
  ...
  #renderLoadingSkeleton(container: HTMLElement) {
    container.innerHTML = `
  ...
  #renderScholarData(container: HTMLElement, data: ScholarResult) {
    container.innerHTML = "";
    ...
    function renderAbstract(abstractEl: HTMLElement, abstract: string) {
  ...
  #renderScholarError(container: HTMLElement, errorMessage: string) {
    container.innerHTML = "";
  ...
  #parseScholarResult(html: string, query: string): ScholarResult | null {
    ...
    const titleEl = firstResult.querySelector(".gs_rt a") as HTMLAnchorElement;
