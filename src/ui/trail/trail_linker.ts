...
import type { PendingConnection, TrailNode } from "./trail_store.js";
 ...
 * @param a normalized title
 * @param b normalized title
 */
...
export class TrailLinker {
  ...
  /**
   * @param originalUrl — the URL this PDF was opened from
   */
  constructor(
    pdfmodel: PDFDocumentModel,
    trailStore: TrailStore,
  ...
  initialize() {
    document.body.addEventListener(
      ...
      (e) => {
        ...
        const popup = link.closest(".citation-popup") as HTMLElement;
        if (!popup) return;
   ...
   * @param destinationTitle — title of the destination paper from Scholar
   */
  async #captureClick(
   ...
   * @returns the matched trail/node, or null
   */
  async matchOnOpen(
    detectedTitle: string | null,
  ): Promise<{ trail: Record<string, any>; node: Record<string, any> } | null> {
    try {
  ...
  async #connectToTrail(
    ...
    const rootNodeData: Omit<TrailNode, "children"> = {
      id: crypto.randomUUID(),
