...
import type { WallpaperPreset } from "./wallpaper.js";
import type { CustomWallpaper, NightModeStartup } from "./config.js";
export class Settings {
  wm: SplitWindowManager;
  showToast: Function;
  wallpaper: WallpaperManager;
  ballStyle: BallEditor;
   ...
   */
  constructor(wm: SplitWindowManager, showToast: Function) {
    this.wm = wm;
  ...
  _setupModalEvents(overlay: HTMLElement) {
    const close = () => {
    ...
    // URL input
    const urlArea = overlay.querySelector("#wallpaper-url-area") as HTMLElement;
    const urlInput = overlay.querySelector<HTMLInputElement>(
      "#wallpaper-url-input",
    )!;
    const urlConfirm = overlay.querySelector<HTMLButtonElement>(
      "#wallpaper-url-confirm",
    )!;
    const urlCancel = overlay.querySelector<HTMLButtonElement>(
      "#wallpaper-url-cancel",
    )!;
    ...
    const wallpaperPersistToggle = overlay.querySelector<HTMLInputElement>(
    ...
    // Progress bar toggle
    const progressToggle = overlay.querySelector<HTMLInputElement>(
      ".progress-bar-toggle",
    );
    if (progressToggle) {
    ...
    const collapseToggle = overlay.querySelector<HTMLInputElement>(
      ".auto-collapse-toggle",
    );
    if (collapseToggle) {
    ...
    // Liquid glass toggle
    const liquidGlassToggle = overlay.querySelector<HTMLInputElement>(
      ".liquid-glass-toggle",
    );
    ...
    // Default tool select
    const defaultToolSelect = overlay.querySelector<HTMLSelectElement>(
      ".default-tool-select",
    );
    ...
    const nightStartupSelect = overlay.querySelector<HTMLSelectElement>(
      ".night-mode-startup-select",
    );
    if (nightStartupSelect) {
      nightStartupSelect.value = Config.get("night_mode_startup");
      nightStartupSelect.addEventListener("change", () => {
        const mode = nightStartupSelect.value as NightModeStartup;
    ...
    const splitPersistToggle = overlay.querySelector<HTMLInputElement>(
      ".split-persist-toggle",
    );
  ...
  _selectWallpaper(id: string | null) {
    if (this._editMode) return;
    this.wallpaper.selectWallpaper(id).then(() => this._refreshGrid());
  }

  _createCardNone(isActive: boolean) {
  ...
  _createCardPreset(preset: WallpaperPreset, isActive: boolean) {
    const card = document.createElement("div");
  ...
  _createCardCustom(entry: CustomWallpaper, isActive: boolean) {
    const card = document.createElement("div");
    card.className =
  ...
  _showAddPopover(anchorCard: HTMLElement) {
    ...
    popover
      ...
      .addEventListener("click", (e) => {
        e.stopPropagation();
        popover.remove();
        const urlArea = this._overlay?.querySelector(
          "#wallpaper-url-area",
        ) as HTMLElement;
        if (urlArea) {
    ...
    const closePopover = (e: MouseEvent) => {
      if (!popover.contains(e.target as Node) && e.target !== anchorCard) {
        popover.remove();
  ...
  _updateDeleteCount() {
    ...
    const span = btn?.querySelector(".delete-count") as HTMLElement;
    if (!btn || !span) return;
  ...
  _truncate(str: string | null | undefined, max: number): string {
