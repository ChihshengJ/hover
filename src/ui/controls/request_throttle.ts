...
class RequestThrottle {
   ...
   * @param key Cache key (typically the search query or a composite like `cite:${title}`)
   * @param fetchFn The actual async work. Only invoked when: - the result is not already cached, AND - there is no identical in-flight request.
   * @param isSuccess Predicate deciding whether the resolved value is worth caching. Results that fail the predicate are returned to the caller but NOT written to the cache, so the next call retries the network. Defaults to truthy.
   */
  async fetch(
    key: string,
    fetchFn: (key: string) => Promise<any>,
    isSuccess: (result: any) => boolean = (r) => Boolean(r),
  ): Promise<any> {
