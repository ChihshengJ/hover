...
export async function buildOutline(pdfDoc, searchIndex, allNamedDests) {
  ...
  if (nativeOutline && nativeOutline.length > 0) {
    ...
    if (nativeOutline.length === 1 && nativeOutline[0].children.length > 1) {
      console.log('[Outline] Single root detected, promoting children to top level');
...
function buildHeuristicOutline(searchIndex) {
  if (!searchIndex?.isBuilt) {
    console.warn('[Outline] Search index not built, cannot generate heuristic outline');
    return [];
  }
  ...
  if (candidates.length > 0) {
    ...
    for (const c of candidates) {
      console.log(`[Outline]   - "${c.title.substring(0, 50)}" (page ${c.pageNumber}, x=${c.x.toFixed(1)}, numbered=${c.isNumbered})`);
    }
  }
  ...
  console.log(`[Outline] Heuristic outline built with ${outline.length} top-level items`);
  return outline;
}
...
function isFullWidthInColumn(line, columns) {
  if (line.columnIndex === -1) return true;
  const tolerance = 0.1;
  const column_width = columns.map((c) => (c.right - c.left));
  const line_width = line.items.reduce((a, curr) => {return a+curr.width}, 0);
...
function isCommonSectionName(text) {
  ...
  // Direct Set lookup (O(1)) - handles exact matches
  if (COMMON_SECTION_NAMES.has(stripped)) return true;
  
  // Check if text starts with a known section name followed by word boundary
  // This handles cases like "Introduction to Deep Learning" matching "introduction"
  // We only check up to reasonable lengths to keep it fast
  const maxCheckLength = 40; // Longest section names are ~30 chars
  const checkText = stripped.length > maxCheckLength 
    ? stripped.slice(0, maxCheckLength) 
    : stripped;
  
  for (const name of COMMON_SECTION_NAMES) {
    if (checkText.length >= name.length && 
 ...
 */
function analyzeLineAsHeading(line, pageNum, bodyFontSize, bodyFontName, columnData) {
  ...
  // Use the pre-computed isAtColumnStart from column-aware line data
  // This properly handles multi-column layouts
  const isAtColumnStart = line.isAtColumnStart === true;
  
  // Check if this is a full-width line (columnIndex === -1)
  ...
  if (isNumbered) {
    ...
    if (!isLargerFont) return null;
  }
  
  console.log(line);
  
  // For non-numbered headings: require larger font AND relatively short text
  const isNonNumberedHeading = !isNumbered && 
    isLargerFont && 
    text.length < 80 && 
    isAtColumnStart;
  
  // Title case check for non-numbered
  const isTitleCaseHeading = !isNumbered && 
    isLargerFont && 
    isTitleCase(text) &&
    isAtColumnStart;

  const isCommonSectionHeading = isCommonSectionName(text);
  
  const isCandidate = isNumbered || isNonNumberedHeading || isTitleCaseHeading || isCommonSectionHeading;
...
function assignLevelsByFontSize(candidates) {
  ...
  const uniqueSizes = [...new Set(candidates.map(c => Math.round(c.fontSize * 10) / 10))]
    .sort((a, b) => b - a);
