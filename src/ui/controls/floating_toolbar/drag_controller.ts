...
import type { GlassEffect } from "./liquid_glass/glass_effect.js";
import type { ViewerPane } from "../../../viewer/viewpane.js";
import type { JumpIndicators, JumpZone } from "./jump_indicators.js";
/** What the drag reports back to the toolbar that owns it. */
export interface DragHooks {
  /** Start of a drag — cancel timers etc. */
  onDragStart(): void;
  /** After a drag ends and the ball has snapped back. */
  onDragEnd(): void;
  /** The horizontal threshold was crossed. */
  onTreeOpenRequested(): void;
  /** The ball was dragged back while the tree was open. */
  onTreeCloseRequested(): void;
}
...
export interface DragControllerOptions {
  ...
  /** Horizontal drag distance, px, that opens the tree. */
  treeOpenThreshold: number;
  /**
   * Optional liquid-glass collaborator; receives the same normalized goo
   * coordinates as the CSS --x/--y custom properties. No-ops when the feature
   * is disabled.
   */
  glassEffect?: GlassEffect | null;
  hooks: DragHooks;
}

export class DragController {
  ball: DragControllerOptions["ball"];
  gooContainer: DragControllerOptions["gooContainer"];
  getPane: DragControllerOptions["getPane"];
  isTreeOpen: DragControllerOptions["isTreeOpen"];
  jumpIndicators: DragControllerOptions["jumpIndicators"];
  treeOpenThreshold: DragControllerOptions["treeOpenThreshold"];
  glassEffect: DragControllerOptions["glassEffect"];
  hooks: DragControllerOptions["hooks"];
  isDragging: boolean;
  isJumping: boolean;
  wasDragged: boolean;
  ...
  _pendingTransform: string | null;
  _pendingGooX: number | null;
  _pendingGooY: number | null;
  _pendingJumpClientY: number | null;
  _pendingTreeOpen: boolean;
   ...
   * @param {GlassEffect} [opts.glassEffect]
   *   Optional liquid-glass collaborator; receives the same normalized goo
  ...
  init() {
    this.ball.addEventListener("pointerdown", (e: PointerEvent) => {
      if (e.button === 0) {
        this.wasDragged = false;
        this.#startDrag(e);
      }
  ...
  executeJump(direction: JumpZone) {
    this.isJumping = true;
  ...
  #computeGooPosition(e: PointerEvent) {
  ...
  #handleHorizontalDrag(deltaX: number) {
  ...
  #handleVerticalDrag(deltaY: number, clientY: number) {
    const deadZone = 10;
